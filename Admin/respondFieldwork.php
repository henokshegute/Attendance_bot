    ...
    if ($list_num > 0) {
        if ($num_of_hours == 'Fullday') {
            if ($num_of_user == 0) {
                ...
                $update_query = "UPDATE attendance_shit SET morning = 'Field Work', morning_in = 'Field Work', morning_out ='Field Work', afternoon='Field Work',afternoon_in='Field Work',afternoon_out='Field Work' WHERE date= '$today' AND chat_id= '$id'";
            ...
            if ($time_of_work == "📝 Morning") {
                if ($num_of_user == 0) {
                    ...
                    $update_query = "UPDATE attendance_shit SET morning = 'Field Work', morning_in = 'Field Work', morning_out ='Field Work' WHERE date= '$today' AND chat_id= '$id'";
                    mysqli_query($con, $update_query);
                    $del = "DELETE FROM field_temp WHERE chat_id='$id'";
                    mysqli_query($con, $del);
                    file_get_contents($botAPI . "/sendmessage?chat_id=" . $id . "&text=Your  request is approved !!!");
                ...
                if ($num_of_user == 0) {
                    ...
                    $update_query = "UPDATE attendance_shit SET afternoon='Field Work',afternoon_in='Field Work',afternoon_out='Field Work' WHERE date= '$today' AND chat_id= '$id'";
                    mysqli_query($con, $update_query);
                    $del = "DELETE FROM field_temp WHERE chat_id='$id'";
                    mysqli_query($con, $del);
                    file_get_contents($botAPI . "/sendmessage?chat_id=" . $id . "&text=Your  request is approved !!!");
