...
if (isset($update->message->text)) {
    ...
    $userrow = mysqli_num_rows($userQ);
    if ($msg == '/start') {
        if ($adminrow == 0 && $userrow == 0) {
            register($chat_id);
        } else if ($adminrow >= 1 && $userrow == 0) {
    ...
    if ($msg == "Morning Checkin") {
        ...
        $morningsign = "SELECT * FROM attendance_shit WHERE date ='$today' AND chat_id = '$chat_id' AND morning is not NULL";
        $morningsignquery = mysqli_query($con, $morningsign);
        $countsigndrows = mysqli_num_rows($morningsignquery);
        if ($countsigndrows > 0) {
    ...
    if ($msg == "Afternoon Checkin") {
        ...
        $afternoonsign = "SELECT * FROM attendance_shit WHERE date ='$today' AND chat_id = '$chat_id' AND afternoon is not NULL";
        $afternoonsignquery = mysqli_query($con, $afternoonsign);
        $countafternooonsigndrows = mysqli_num_rows($afternoonsignquery);
        if ($countafternooonsigndrows > 0) {
            $errormessage = "You have alrady signed your  attendance";
            file_get_contents($botAPI . "/sendMessage?text=$errormessage&chat_id=$chat_id&parse_mode=html");
            returnmenu($chat_id);
        } else {
            $morningsignforafternoon = "SELECT * FROM attendance_shit WHERE date ='$today' AND chat_id = '$chat_id' AND morning is not NULL";
            $morningsignforafternoonquery = mysqli_query($con,  $morningsignforafternoon);
            $countmorningsignforafternoon = mysqli_num_rows($morningsignforafternoonquery);
            if ($countmorningsignforafternoon > 0) {
    ...
    if ($msg == "Morning Checkout") {
        ...
        $time = date("h:ia");
        $today = date('y-m-d');
        $mornongincheck = "SELECT * FROM attendance_shit WHERE date ='$today' AND chat_id = '$chat_id' AND morning is not NULL";
        $mornongincheckquery = mysqli_query($con,  $mornongincheck);
        $countmornongincheckquery = mysqli_num_rows($mornongincheckquery);
        if ($countmornongincheckquery> 0) {
    ...
    if ($msg == "Afternoon Checkout") {
        ...
        $time = date("h:ia");
        $today = date('y-m-d');
        $afternoonincheck = "SELECT * FROM attendance_shit WHERE date ='$today' AND chat_id = '$chat_id' AND afternoon is not NULL";
        $afternoonincheckquery = mysqli_query($con, $afternoonincheck);
        $afternoonsigncheck = mysqli_num_rows($afternoonincheckquery);
    ...
    if ($update->message->reply_to_message) {
        $startingRangeCompanyLatitude = 8.989430;
        $endingRangeCompanyLatitude = 8.989870;
        $startingRangeCompanyLongitude = 38.786250;
        $endingRangeCompanyLongitude = 38.786630;
